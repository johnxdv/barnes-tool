...
export function EstimationLoadingStep({ onDone, onProgress }) {
  ...
  return (
    /* Le module d'attente est centré sur l'écran, et la vignette à l'encre est
       posée à côté de lui — pas dans la même grille.

       Les deux occupaient auparavant deux colonnes d'une même grille, et c'est
       ce qui décentrait l'écran : une colonne de seize rem à droite pousse tout
       le reste vers la gauche, et le titre, la barre et la liste d'étapes se
       lisaient en bord de fenêtre. La vignette est maintenant accrochée au
       flanc droit du module (`left-full`, en absolu) : elle ne prend aucune
       place dans le flux, et le module retrouve le milieu exact de l'écran —
       `justify-center` dans `App` fait le reste.

       Elle ne paraît qu'à partir de 1280 px : en dessous, le module centré
       laisse trop peu de marge à sa droite, et une vignette de seize rem
